...
In a balanced system the number of clients that can be handled in a specific time unit will match the average number of arriving clients in that time unit. The offered load, $\rho$, is defined as:

$$
\rho=\frac{\lambda}{\mu}
$$

The unit of $\rho$ is Erlang and is dimensionless.
...
print(P)
fig = px.line(x=u, y=P, title=title)
fig.show()
```
...
\<iframe width="560" height="315" src="https://www.youtube.com/embed/3mPA9x9-PWI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen\>\</iframe\>
