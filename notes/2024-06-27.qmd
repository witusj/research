...
Run a simple test instance of a schedule.
...
Generate all feasable schedules with $T = 3$ and $N = 6$ and calculate loss values.
...
df = pd.DataFrame.from_dict(data).sort_values(by=['loss'], ascending=False, ignore_index=True)
df
```
...
3.  **Reshape Data**:
    ...
    -   `1`: This represents the number of channels. Since your data isn't color-coded and only consists of numerical values for each feature, we use 1 channel.
    ````
...
5.  **Build the Model**:
    ...
    -   Adds a `Conv2D` layer with 64 filters, a kernel size of `(2, 1)`, and ReLU activation. The input shape is specified as `(3, 1, 1)`.
    -   Adds a `Flatten` layer to convert the 2D output of the convolutional layer to a 1D array.
    -   Adds a `Dense` layer with 64 units and ReLU activation.
    -   Adds an output `Dense` layer with 3 units (one for each target variable).
...
10. **Make Predictions**: `python     # Make predictions     predictions = model.predict(X_test)`
