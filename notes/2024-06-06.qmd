...
The problem can be illustrated as a landscape that contains all possible solutions (points) and the task is to find a path that leads to the solution (point) that has the lowest value. Each point can be defined by an array of T coordinates with values between 0 and N, such that the sum of all coordinates equals N.
...
1. The objective function of a model with fixed and equal time slots is multimodular relatively to schedule $x$.
2. In the case where appointment times are continuous and variable the optimal schedule is dome-shaped.


<ol class="custom-counter">
    <li>....</li>
    <li>Would that mean that schedules with overbooking at the start and towards the finish and more zeroes in the middle show optimal performance?</li>
    <li></li>
