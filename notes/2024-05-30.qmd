...
We denote total waiting times as follows:
- \( a_1 \) for schedule (A1) until time \( j-1 \)
- \( a_2 \) for schedule (A2) until time \( j-1 \)
- \( b_1 \) for schedules (A1) and (A2) after time \( j-1 \)
- \( b_2 \) for schedules (A3) and (A4) after time \( j-1 \)

From these, we establish the equality:
\[ a_2 + b_1 + a_1 + b_2 = a_1 + b_1 + a_2 + b_2 \]
