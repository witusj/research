...
fig = go.Figure(data=go.Heatmap(
    z=Q,
    x=['State 1', 'State 2', 'State 3'],  # Label states for clarity
    y=['State 1', 'State 2', 'State 3'],
    colorscale='Viridis'  # Color scale similar to matplotlib's 'viridis'
))

# Update layout for better visualization
fig.update_layout(
    title='CTMC Transition Rate Matrix Q',
...
# Create two heatmaps in a single figure
fig = go.Figure()
...
# Update layout for a side-by-side view
fig.update_layout(
    title='CTMC Transition Rate Matrix Q and Subgenerator Matrix S',
    grid=dict(columns=2, rows=1),  # Setting the grid for side-by-side layout
    xaxis=dict(title='To State', domain=[0, 0.40]),  # Domain specifies the fraction of the layout for this subplot
    yaxis=dict(title='From State'),
    xaxis2=dict(title='To State', domain=[0.57, 1]),  # Domain specifies the fraction of the layout for this subplot
    yaxis2=dict(title='From State', anchor='x2')  # Anchor aligns this axis with the x2 axis
)
...
# Create a Plotly heatmap to visualize the state probabilities
fig = go.Figure(data=go.Heatmap(
    z=P_t,
    x=[f'State {i}' for i in range(len(P_t))],
    y=[f'State {i}' for i in range(len(P_t))],
    colorscale='Reds'
))

# Update layout for better visualization
fig.update_layout(
    title=f'State Probabilities at Time t={t}',
    xaxis_title='State at Time t',
    yaxis_title='Initial State',
    xaxis=dict(tickmode='array', tickvals=list(range(len(P_t)))),
    yaxis=dict(tickmode='array', tickvals=list(range(len(P_t))))
)
...
fig = go.Figure(data=go.Histogram(
    ...
    marker_color='purple',  # Color of the histogram
    opacity=0.7  # Transparency of the histogram
))

# Update layout for better visualization
fig.update_layout(
    title='Histogram of Erlang Distributed Data',
...
# Add histogram for interarrival times
fig.add_trace(go.Histogram(
    x=interarrival_times,
    nbinsx=20,  # Number of bins
    marker_color='blue',  # Color of the histogram bars
    opacity=0.7,  # Transparency of the histogram bars
    name='Interarrival Times'  # Legend name
))

# Add histogram for service times
fig.add_trace(go.Histogram(
    x=service_times,
    nbinsx=20,  # Number of bins
    marker_color='red',  # Color of the histogram bars
    opacity=0.7,  # Transparency of the histogram bars
    name='Service Times'  # Legend name
))

# Update the layout to display the histograms side by side
fig.update_layout(
    title='Histograms of Interarrival and Service Times',
    xaxis=dict(
        domain=[0, 0.45],  # Domain for the first subplot
        title='Time'
    ),
    xaxis2=dict(
        domain=[0.55, 1],  # Domain for the second subplot, with space for separation
        title='Time'
    ),
    yaxis_title='Frequency',
...
fig = go.Figure(data=go.Histogram(
    ...
    marker_color='blue',  # Color of the histogram
    opacity=0.75  # Transparency of the histogram
))

# Update layout for better visualization
fig.update_layout(
    title="Histogram of Generated Erlang Distribution Data",
    xaxis_title="Data Values",
    yaxis_title="Frequency",
    template='plotly_white'
)
...
shape = 3  # Original number of phases in series
scale = 2  # Original rate parameter
size = 500  # Number of data points
data = np.random.gamma(shape, scale, size)
...
# Estimate Erlang k (shape) as (mean^2 / variance)
...
# Create the figure for both histogram and PDF line
fig = go.Figure()

# Add histogram for the data
fig.add_trace(go.Histogram(
    x=data,
    nbinsx=30,  # Number of bins
    marker_color='blue',  # Color of the histogram
    opacity=0.5,  # Transparency of the histogram
    name='Data Histogram',  # Legend name
    histnorm='probability'  # Normalize the histogram
))

# Add line plot for the estimated Erlang PDF
fig.add_trace(go.Scatter(
    x=x,
    y=erlang.pdf(x, a=estimated_k, scale=1/estimated_lambda),
    mode='lines',
    line_color='red',
    name='Fitted Erlang PDF'
))

# Update the layout for better visualization
fig.update_layout(
    title="Erlang Fitting Using Method of Moments",
    xaxis_title="Data Values",
...
# Assuming 'data' is already generated and available
...
fig.add_trace(go.Histogram(
    ...
    nbinsx=30,  # Number of bins
    marker_color='blue',  # Color of the histogram bars
    opacity=0.5,  # Transparency of the histogram bars
    name='Data Histogram',  # Legend name
    histnorm='probability'  # Normalize the histogram
))

# Add line plot for the MLE fitted Erlang PDF
fig.add_trace(go.Scatter(
    x=x,
    y=erlang.pdf(x, a=int(best_fit_shape), scale=1/best_fit_scale),
    mode='lines',
    line_color='green',
    name='MLE Fitted Erlang PDF'
))

# Update the layout for better visualization
fig.update_layout(
    title="Erlang Fitting Using MLE",
    xaxis_title="Data Values",
    yaxis_title="Probability Density",
    legend_title="Legend",
    template='plotly_white',
