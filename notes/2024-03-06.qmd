...
We have $\dbinom{N+T-1}{N}$. As $N+T$ is the larger factor it will determined the complexity. 
...
The products $T!$ and $(N+T)^T$ have the same number of terms. However as $N+T >> T$, the size of the problem will grow with $\mathcal{O}((N+T)^T)$
