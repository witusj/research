...
class Schedule:
    ...
    def impute_compositions(self, s=1, k=2):
      ...
      store_optim = dict({ 'x' : self.parameters['x'].copy(), 'ew' : self.system["ew"].copy(), 'tot_wt' : lowest_wt})
      ...
      for comp in compositions:
        ...
        if(test_wt < lowest_wt):
          ...
          store_optim['ew'] = self.system["ew"].copy()
          store_optim['tot_wt'] = lowest_wt
        logging.info(f'{datetime.datetime.now()} - wt = {self.system["ew"]} (total = {self.system["ew"].sum()}) for x = {self.parameters["x"]}')
      logging.info(f'{datetime.datetime.now()} - lowest wt = {store_optim["tot_wt"]} for x = {store_optim["x"]}')
      self.parameters['x'] = store_optim['x']
      self.system["ew"] = store_optim['ew']
...
def run_schedule(x, d, s, u=1, print_system=True):
  ...
  k = 3
  for i in np.arange(1, (len(x) - k)):
    schedule.impute_compositions(s=i, k=k)
  return(schedule)
```
